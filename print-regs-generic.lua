...
-- Strip any trailing spaces and tabs, then append a newline. We don't want
-- to strip out *all* whitespace, because sometimes we want extra newlines!
function out(format, ...)
    local s = fmt(format, ...)
    s = s:gsub("[ 	]+$", "")
    -- s = s:gsub("%s+$", "")
...
function print_header(c, name, srcpath)
    ...
    out("| from CMSIS-SVD source file %s", srcpath)
    out("| version %s\n", c.chip.version)
...
function print_vectors(c)
    local print_vector = function(vecfmt, name, vector, description)
        ...
        if not description then
            description = fmt("IRQ %2d", vector)
        else
            description = fmt("IRQ %2d: %s", vector, description)
        end
        out(vecfmt,
            (vector + 16) * 4,
            name,
    ...
    local vecfmt = fmt("%%04x vector %%-%ds | %%s", longest + 3)
...
function print_periphs(c)
    ...
    local function add_cluster(reg, regs)
        ...
        if array_present == 1 then
            ...
            local descr = fmt("%s (%d %s)",
                remove_array(reg.description), count, units)

            append(regs, { name = name,
                           address_offset = reg.address_offset,
                           descr = descr } )
        else
            -- true cluster
